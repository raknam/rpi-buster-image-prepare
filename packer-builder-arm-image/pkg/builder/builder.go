...
func (b *Builder) Prepare(cfgs ...interface{}) ([]string, []string, error) {
	...
	if b.config.OutputFile == "" {
		if b.config.OutputDir != "" {
			...
			b.config.OutputFile = filepath.Join(b.config.OutputDir, "image.img")
		} else {
			b.config.OutputFile = fmt.Sprintf("output-%s/image.img", b.config.PackerConfig.PackerBuildName)
		}
	}
